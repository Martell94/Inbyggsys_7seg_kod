...
static timer_t timer_digit;
static timer_t timer_count_speed;
...
void display_enable_output(void){
   ...
   eeprom_write_byte(EEPROM_OUTPUT_ENABLED, 1); //Sparar att displayerna är på till EEPROM.
   display_enable_count(); //Återaktiverar upp resp. nedräkning efter displayer varit släckta. 
...
void display_disable_output(void){
   ...
   eeprom_write_byte(EEPROM_OUTPUT_ENABLED, 0); //Sparar att displayerna är av till EEPROM.
   display_disable_count(); //Avbryter räkning efter att man släckt displayerna. 
   return;
}
...
void display_toggle_count_direction(void){
   ...
   eeprom_write_byte(EEPROM_COUNT_DIRECTION, (uint8_t)(count_direction)); //Sparar uppräkningsriktning. 
   return;
}
